...
description: Gets GLSL source code either provided as strings or can load from
filenames, compiles the shaders, creates a shader program which the shaders are
linked to, then the program is validated and is ready for use via
myProgram.use(), <draw-stuff-here> then calling myProgram.disable();
...
class ShaderProgram {
public:
	// static DEBUG flag - if set to false then, errors aside, we'll run
	// completely silent
	static const bool DEBUG = true;

private:

	// We'll use an enum to differentiate between shaders and shader programs when
	// querying the info log
	...
	GLuint compileShader(std::string shaderSource, GLenum shaderType) {
		...
		switch (shaderType) {
			...
			case GL_GEOMETRY_SHADER:
				throw std::runtime_error(
						"Geometry shaders are unsupported at this time.");
				break;
			default:
		...
		if (shaderStatus == GL_FALSE) {
			throw std::runtime_error(shaderTypeString + " compilation failed: " +
									 getInfoLog(ObjectType::SHADER, shaderId));
		}/* else {
			if (DEBUG) {
	...
	// Note: Rather than returning a boolean as a success/fail status we'll just
	// consider a failure here to be an unrecoverable error and throw a
	// runtime_error.
	void initialise(std::string vertexShaderSource,
		...
		// Check the program link status and throw a runtime_error if program
		// linkage failed.
...
public:
	ObjectType getTypeProgram() {
		return ObjectType::PROGRAM;
	}
	// Constructor
	ShaderProgram() {
		// We start in a non-initialised state - calling initFromFiles() or
		// initFromStrings() will initialise us.
		initialised = false;

		// Generate a unique Id / handle for the shader program
		// Note: We MUST have a valid rendering context before generating the
		// programId or we'll segfault!
	...
	// Method to return the bound location of a named attribute, or -1 if the
	...
	GLuint attribute(const std::string attributeName) {
		...
		//
		// BUT, if you did, and you asked it for a named attribute which didn't
		// exist like: attributeMap["FakeAttrib"] then the method would return an
		// invalid value which will likely cause the program to segfault. So we're
		// making sure the attribute asked for exists, and if it doesn't then we
		// alert the user & bail.

		// Create an iterator to look through our attribute map (only create
		// iterator on first run - reuse it for all further calls).
		...
		if (attributeIter == attributeMap.end()) {
			throw std::runtime_error("Could not find attribute in shader program: " +
									 attributeName);
		}
	...
	GLuint uniform(const std::string uniformName) {
		...
		// Create an iterator to look through our uniform map (only create iterator
		// on first run - reuse it for all further calls).
		...
		if (uniformIter == uniformMap.end()) {
			throw std::runtime_error("Could not find uniform in shader program: " +
									 uniformName);
		}
	...
	int addUniform(const std::string uniformName) {
		...
		if (uniformMap[uniformName] == -1) {
			throw std::runtime_error("Could not add uniform: " + uniformName +
									 " - location returned -1.");
		} else // Valid uniform location? Inform user if we're in debug mode.
		{
			if (DEBUG) {
				std::cout << "Uniform " << uniformName
						  << " bound to location: " << uniformMap[uniformName]
						  << std::endl;
			}
		}
